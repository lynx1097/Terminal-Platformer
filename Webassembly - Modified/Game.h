...
private:
    ...
    Hero* hero;
    string statusMsg;
    // Add these to private members of Game
    int gameState = 0; // 0 = Choose Hero, 1 = Choose Gun 1, 2 = Choose Gun 2, 3 = Playing
    int selectedHero = 0;
    int selectedGun1 = 0;
...
public:
    ...
    // ── Start screen ─────────────────────────────────────────────────────────
    // Replace startGame() with these two methods:
    ...
    // THIS is the function JavaScript will call every time a key is pressed
    void handleInput(char input) {
        if (gameState == 0) {
            if (input == '1' || input == '2') {
                selectedHero = input - '0';
                cout << input << "\n" << col::LABEL << "  Choose weapon " << col::VALUE << "[1] Pistol  [2] Bazooka  [3] Rifle" << col::RESET << "  > ";
                gameState = 1;
            }
        } 
        else if (gameState == 1) {
            if (input >= '1' && input <= '3') {
                selectedGun1 = input - '0';
                cout << input << "\n";
                if (selectedHero == 1) {
                    cout << col::LABEL << "  Second weapon " << col::VALUE << "[1] Pistol  [2] Bazooka  [3] Rifle" << col::RESET << "  > ";
                    gameState = 2;
                } else {
                    setupHeroAndStart(0); // Skip second gun for Medic
                }
            }
        }
        else if (gameState == 2) {
            if (input >= '1' && input <= '3') {
                setupHeroAndStart(input - '0');
            }
        }
        else if (gameState == 3) { // PLAYING
            if (checkGameOver() != -1) return; // Game is over
    ...
    // Helper to spawn the hero once selections are made
    void setupHeroAndStart(int gun2Type) {
        Gun* gun1 = (selectedGun1 == 1) ? (Gun*)new Pistol() : (selectedGun1 == 2) ? (Gun*)new Bazooka() : (Gun*)new Rifle();
        Gun* gun2 = nullptr;
        if (gun2Type == 1) gun2 = new Pistol();
        else if (gun2Type == 2) gun2 = new Bazooka();
        else if (gun2Type == 3) gun2 = new Rifle();
        ...
        generateMap();
        grid[0][0] = hero;
        gameState = 3; // Switch to PLAYING state
        updateMap();
    }
};
